...
<?php
            if(isset($_POST['tambahkasir']))
            {
                ...
                $tambahkat = mysqli_query($conn,"INSERT INTO login (username, alamat, telepon, password)
                values ('$username','$alamat','$telpon','$password')");
                if ($tambahkat){
                ...
                $result = mysqli_query($conn, "UPDATE login SET username='$user', password='$pass',alamat='$alamat',telepon='$telp'
...
<?php 
    if(!empty($_GET['hapus'])){
        ...
        if($hapus_data){
            echo '<script>alert("Berhasi Hapus kasir");window.location="pengaturan.php"</script>';
        } else {
